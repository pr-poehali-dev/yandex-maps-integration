...
def handler(event: dict, context) -> dict:
    ...
    if action == 'profile':
        ...
        cur.execute("SELECT u.id, u.name, u.email, u.is_wholesale, u.created_at FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = %s AND s.expires_at > NOW()", (token,))
        row = cur.fetchone()
        if not row:
            cur.close(); conn.close(); return err('Сессия истекла', 401)

        user_id, name, email, is_wholesale, created_at = row
        cur.execute("SELECT card_number, discount_percent, total_purchases FROM discount_cards WHERE user_id = %s", (user_id,))
        card = cur.fetchone()
        cur.close(); conn.close()
        return ok({'id': user_id, 'name': name, 'email': email, 'is_wholesale': is_wholesale, 'member_since': created_at.strftime('%d.%m.%Y'), 'card': {'number': card[0], 'discount_percent': card[1], 'total_purchases': card[2]} if card else None})
